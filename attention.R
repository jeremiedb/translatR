...
attention_dot <- function(query, key, value, scaled = T, weighting = F, num_hidden, num_proj_key = NULL, query_proj_weight = NULL) {
  
  # query: either a copy of last hidden or projection of it
  if (!is.null(num_proj_key)) {
    query <- mx.symbol.FullyConnected(data = query, num_hidden = num_proj_key, weight = query_proj_weight, no_bias = T, flatten = F)
    ...
    if (scaled) query = query / sqrt(num_hidden)
