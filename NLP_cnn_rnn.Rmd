...
```{r}
buckets <- read_rds("data/buckets_euro_en_fr_8_48.rds")
...
```{r}
mx.set.seed(44)
iter_train <- mx.io.arrayiter(data = buckets$buckets$`48`$data, label = buckets$buckets$`48`$label, batch.size = batch_size, shuffle = T)
...
lr_scheduler <- mx.lr_scheduler.FactorScheduler(step = 2000, factor_val = 0.9, stop_factor_lr = 5e-5)
...
mx.model.save(model = model, prefix = "models/model_euro_en_fr_cnn_rnn_teacher_v1", iteration = 10)
...
```{r}
model <- mx.model.load(prefix = "models/model_euro_en_fr_cnn_rnn_teacher_v1", iteration = 10)
