...
```{r}
mx.set.seed(44)
iter_train <- mx.io.arrayiter(data = buckets$buckets$`64`$data, label = buckets$buckets$`64`$label, batch.size = batch_size, shuffle = T)
...
mx.model.save(model = model, prefix = "models/model_wmt15_en_fr_cnn_rnn_teacher_v1", iteration = 8)
mx.symbol.save(symbol = decode_argmax, filename = "models/model_wmt15_en_fr_cnn_rnn_argmax_v1.json")
```
...
```{r}
model <- mx.model.load(prefix = "models/model_wmt15_en_fr_cnn_rnn_teacher_v1", iteration = 8)
sym_infer <- mx.symbol.load(file.name = "models/model_wmt15_en_fr_cnn_rnn_argmax_v1.json")
...
```{r}
ctx <- mx.gpu()
bucket_test <- read_rds("data/buckets_wmt14_eval_en_fr_64.rds")
batch_size <- 32
seq_len <- 64
...
write_lines(translation, path = "data/translation/wmt14_en_fr_rnn_cnn_v2.txt")
```
