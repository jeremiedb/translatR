...
# special_en <- grep(pattern = "<|>", x = raw_en)
# special_fr <- grep(pattern = "<|>", x = raw_fr)
# 
# raw_en[special_en]
# raw_fr[special_en]
# 
...
raw_en <- gsub(pattern = "’", replacement = "'", x = raw_en)
raw_fr <- gsub(pattern = "’", replacement = "'", x = raw_fr)

chars <- stri_split_boundaries(raw_fr[1:100000], type = "character", simplify = F)
chars <- unlist(chars)
sort(unique(chars))
```
...
create_buckets <- function(source, target, 
  ...
  buckets = list("48" = list(data = source, label = target))
...
write_rds(buckets, "data/buckets_euro_en_fr_8_48.rds")
```
