...
```{r}
raw_en <- read_lines("data/europarl-v7.fr-en.en")
...
raw_en[1282000:1282005]
raw_fr[1282000:1282005]
```
...
corpus_pre_process <- function(source, 
  ...
  source_word_vec_list <- stri_split_boundaries(source, type = "character", 
                                                simplify = F)
  
  target_word_vec_list <- stri_split_boundaries(target, type = "character", 
                                                simplify = F)
  
  gc()
  
  # add BOS-EOS tokens
  source_word_vec_list <- lapply(source_word_vec_list, function(x) c("<BOS>", x, "<EOS>"))
  target_word_vec_list <- lapply(target_word_vec_list, function(x) c("<BOS>", x, "<EOS>"))
...
# length 1644721
preprocess <- corpus_pre_process(source = raw_en[1:1000000], 
                                 target = raw_fr[1:1000000], 
                                 min_seq_len = 20, 
                                 max_seq_len = 200, 
                                 min_seq_ratio = 2/3, 
                                 max_seq_ratio = 3/2,
...
# write_rds(preprocess, path = "data/preprocess_euro_en_fr_8_48.rds")
# rm(preprocess)
rm(raw_en)
rm(raw_fr)
gc()
...
create_buckets <- function(source, target, 
  ...
  buckets = list(data = source, label = target)
...
```{r, eval = FALSE}
# preprocess <- read_rds(path = "data/preprocess_euro_en_fr_8_48.rds")
...
write_rds(buckets, "data/buckets_euro_en_fr_char_20_200.rds")
