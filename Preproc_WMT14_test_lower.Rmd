...
```{r}
raw_en <- read_lines("data/wmt14-en-fr.src")
...
dic <- read_rds("models/model_euro_en_fr_cnn_rnn_v2_dic.rds")
...
create_buckets <- function(source, target, 
  ...
  buckets = list("72" = list(data = source, label = target))
...
```{r, eval = FALSE}
# preprocess <- read_rds(path = "data/wmt14_test_en_fr.rds")
...
head(preprocess$source_dt,20)

source_dic = preprocess$dic$source_dic
head(preprocess$dic$source_dic[20000:20020])

data <- matrix(0, nrow = 72, ncol = ncol(buckets$buckets$`72`$data))
data[1:71, ] <- buckets$buckets$`72`$data

label <- matrix(0, nrow = 72, ncol = ncol(buckets$buckets$`72`$label))
label[1:71, ] <- buckets$buckets$`72`$label

buckets$buckets$`72`$data <- data
buckets$buckets$`72`$label <- label

write_rds(buckets, "data/buckets_wmt14_test_en_fr_72.rds")
