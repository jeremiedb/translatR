...
mx.model.train.buckets <- function(symbol, ctx, train.data, eval.data, 
  ...
  # })
  slices <- lapply(names(dlist), function(i) {
    mx.nd.split(data=dlist[[i]], num_outputs = ndevice, axis = 0, squeeze_axis = F)
  })
  
  train.execs <- lapply(1:ndevice, function(i) {
    if (ndevice == 1) s <- dlist else s <- lapply(slices, function(slice) slice[[i]])
    names(s) <- names(dlist)
  ...
  for (iteration in begin.round:end.round) {
    ...
    while (train.data$iter.next()) {
      ...
      if (is.list(symbol)) {
        train.execs <- lapply(1:ndevice, function(i) {
          ...
          mxnet:::mx.symbol.bind(symbol = symbol[[names(train.data$bucketID)]], 
                                 arg.arrays = c(s, train.execs[[i]]$ref.arg.arrays[arg.params.names])[arg.update.idx],
                                 aux.arrays = train.execs[[i]]$ref.aux.arrays, ctx = ctx[[i]], grad.req = grad.req)
        })
      } else {
        for (i in 1:ndevice) {
          if (ndevice == 1) s <- dlist else s <- lapply(slices, function(slice) slice[[i]])
      ...
      # if (nbatch %% 5 == 0) gc()
      if (nbatch %% 250 == 0 & !is.null(metric)) {
        train.metric <- metric$init()
      }
...
mx.model.buckets <- function(symbol, train.data, eval.data = NULL, metric = NULL,
                             ...
                             kvstore = "local", verbose = TRUE, metric_cpu = FALSE) {
  ...
  input.shape <- sapply(input.names, function(n) {
    shape <- dim(train.data$value()[[n]])
    shape[length(shape)] <- shape[length(shape)] / ndevice
    shape
  }, simplify = FALSE)
  ...
  dlist <- train.data$value()[input.names][input.names]
  ...
  kvstore <- mxnet:::mx.model.create.kvstore(kvstore, arg.params, length(ctx), 
                                             verbose = verbose)
